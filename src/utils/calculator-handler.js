...
export const calculatorHandler = ({ keypadValue, entry, expression }) => {
    ...
    const calculatorOperations = {
        ...
        [CALCULATOR_BUTTONS.backSpace]: () => {
            if (entry) {
                ...
                newState.expression = (expression.slice(0, expr.length - 1) || '');
            }
        },
        ...
        operator: (keypadValue) => {
            ...
            if (
                ...
                if (
                    ...
                    if (keypadValue === CALCULATOR_BUTTONS.plus ||
    ...
    if (Number(entry) === Infinity || isNaN(entry) && entry !== CALCULATOR_BUTTONS.dot) {
        entry = '';
        newState.entry = '';
    }
